...
public class StringUtils {
	...
	public static Charset toCharset(String charsetName) {
		if (charsetName != null) {
			try {
				...
				if(log.isErrorEnabled()) log.error("Convert charset failed", t);
			}
		}
		return null;
	}
	 ...
	 *
	 * @param phoneNo 휴대폰번호
	 * @return 3부분으로 분리된 문자열 배열
	 */
	public static String[] splitPhoneNo(String phoneNo) {
		Pattern pattern = Pattern.compile("^(01\\d{1}|02|0505|0502|0506|0\\d{1,2})-?(\\d{3,4})-?(\\d{4})");
		Matcher matcher = pattern.matcher(numberChar(phoneNo));
		...
		// 정규식 미일치
			...
			String numPhoneNo = numberChar(phoneNo);

			if (numPhoneNo.length() == 9) {
				String phoneNo1 = numPhoneNo.substring(0, 2);
				String phoneNo2 = numPhoneNo.substring(2, 5);
				String phoneNo3 = numPhoneNo.substring(5, numPhoneNo.length());

				return new String[] { phoneNo1, phoneNo2, phoneNo3 };
			}
			else if (numPhoneNo.length() == 10) {
				String phoneNo1 = numPhoneNo.substring(0, 3);
				String phoneNo2 = numPhoneNo.substring(3, 6);
				String phoneNo3 = numPhoneNo.substring(6, numPhoneNo.length());

				return new String[] { phoneNo1, phoneNo2, phoneNo3 };
			}
			else {
				String phoneNo1 = numPhoneNo.substring(0, 3);
				String phoneNo2 = numPhoneNo.substring(3, 7);
				String phoneNo3 = numPhoneNo.substring(7, numPhoneNo.length());
